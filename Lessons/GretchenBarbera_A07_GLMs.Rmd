...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(
  x = plotID, y = dryMass)) +
  ...
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = 
                 c("abc", "ab", "abcd", "bcd", "bcd", "a", "d", "abcd", "cd", "d", "abcd", "abcd")) +
  labs(x = "Plot ID", y = "Dry Mass (g)") +
  ylim(0, 85)
print(Litter.Totals.plot)
...
Litter.Totals <- Litter.Totals %>%
  mutate(label = c("abc", "ab", "abcd", "bcd", "bcd", "a", "d", "abcd", "cd", "d", "abcd", "abcd"),
         label_present = ifelse(label %in% c("abc", "ab", "abcd", "bcd", "a", "d", "cd"), TRUE, FALSE))



Litter.Totals <- Litter.Totals %>%
  mutate(plotID = fct_reorder(plotID, dryMass, .fun = median))
