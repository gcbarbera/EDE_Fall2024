...
glimpse(stream_gage_NE)
st_crs(stream_gage_NE)

st_crs(stream_gage_NE)$epsg


#5. Plot the data

ggplot(data = stream_gage_NE) +
  geom_sf(fill = "lightblue", color = "darkblue")


mapview(stream_gage_NE)
...
> ANSWER: 
> The EPSG is 4269. Uses latitude and longitude
...
>ANSWER: 
>The sf data object gained the geometry field. The lat and long columns are not longer in the sf dataset. 
...
#the lat and long on the plot looks awesome 
```
...
#before joining- I have to make sure they have the same site_no
...
colnames(gage_locations_with_flow)
...
ggplot() +
  geom_sf(data = stream_gage_NE, fill ="lavender", color = "darkblue") +
  geom_sf(data = gage_locations_with_flow, aes(color = gage_ht))
